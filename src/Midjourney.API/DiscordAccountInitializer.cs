        ...
        public async Task StartCheckAccount(DiscordAccount account, bool isValidateLock = true)
        {
            ...
            var isLock = await AsyncLocalLock.TryLockAsync($"initialize:{account.Id}", TimeSpan.FromSeconds(5), async () =>
            {
                ...
                try
                {
                    ...
                    sw.Stop();
                    info.AppendLine($"[{account.GuildId}] 初始化中... 获取任务数耗时: {sw.ElapsedMilliseconds}ms");
                    sw.Restart();
                    ...
                    if (setting.EnableAutoExtendToken && !account.IsYouChuan && !account.IsOfficial)
                    {
                        ...
                        sw.Stop();
                        info.AppendLine($"[{account.GuildId}] 初始化中... 随机延期token耗时: {sw.ElapsedMilliseconds}ms");
                        sw.Restart();
                    }
                        ...
                        if (disInstance == null)
                        {
                            ...
                            if (setting.EnableAutoGetPrivateId && !account.IsYouChuan && !account.IsOfficial)
                            {
                                ...
                                catch (Exception ex)
                                {
                                    ...
                                    info.AppendLine($"[{account.GuildId}] 初始化中... 获取 MJ 私聊频道 ID 异常");
                                }

                                sw.Stop();
                                info.AppendLine(
                                    $"[{account.GuildId}] 初始化中... 获取 MJ 私聊频道 ID 耗时: {sw.ElapsedMilliseconds}ms");
                                sw.Restart();

                                try
                                {
                                    Thread.Sleep(500);
                                    var id = await _discordAccountHelper.GetBotPrivateId(account,
                                        EBotType.NIJI_JOURNEY);
                                    if (!string.IsNullOrWhiteSpace(id))
                                    {
                                ...
                                catch (Exception ex)
                                {
                                    ...
                                    info.AppendLine($"[{account.GuildId}] 初始化中... 获取 NIJI 私聊频道 ID 异常");
                                }

                                sw.Stop();
                                info.AppendLine(
                                    $"[{account.GuildId}] 初始化中... 获取 NIJI 私聊频道 ID 耗时: {sw.ElapsedMilliseconds}ms");
                                sw.Restart();
                            }

                            account.DayDrawCount = dayCount;
                            db.Update(
                                "NijiBotChannelId,PrivateChannelId,AllowModes,SubChannels,SubChannelValues,FastExhausted,DayDrawCount",
                                account);
                            ...
                            if (account.EnableRelaxToFast == true && !account.IsYouChuan && !account.IsOfficial)
                            {
                                ...
                                sw.Stop();
                                info.AppendLine(
                                    $"[{account.GuildId}] 初始化中... 慢速切换快速模式检查耗时: {sw.ElapsedMilliseconds}ms");
                                sw.Restart();
                            }
                ...
                finally
                {
                    ...
                    info.AppendLine($"[{account.GuildId} {account.Remark}] - 频道池初始化完成:[ {string.Join(",", disInstance?.AllChannelIds)} ]");
                    info.AppendLine("======================End==========================");
